...
Most documents carry their own `# ` heading, and the proposal derives the manifest title from that heading, so the rendered page would print the title twice. After parsing, the script drops the article's first `<h1>` when its text matches the header's. It is the first `<h1>` anywhere in the article, not the article's first element: a document may open with a banner line or other preamble, and anchoring on the first element missed the heading in those. The removal happens in the browser because the embedded Markdown stays byte-exact: stripping the heading from the source would break the round trip that `apply`'s byte check and the diff both rest on. A document whose heading differs from its manifest title keeps both, which is the case where the two really do say different things — so a source whose heading changes needs its manifest title updated in the same run.
